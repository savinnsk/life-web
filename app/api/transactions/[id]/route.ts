...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        const currentTransaction = await dbMethods.get('SELECT * FROM transactions WHERE id = ?', [id]) as Transaction;
        ...
        if (currentTransaction.is_fixed) {
           ...
           WHERE parent_transaction_id = ? OR id = ?`,
                [description, amount, type, category, paid, currentTransaction.parent_transaction_id || id, id]
            );
        } else {
            // Transação normal
            await dbMethods.run(
           ...
           WHERE id = ?`,
                [description, amount, type, category, date, paid, is_fixed || 0, id]
            );

            // Se foi marcado como fixo agora, criar para os próximos meses
            if (is_fixed && !currentTransaction.is_fixed) {
                await createFixedTransactions(id, description, amount, type, category, date);
            }
        }
    ...
    try {
        ...
        if (transaction) {
            ...
            if (transaction.is_fixed) {
                ...
                await dbMethods.run('DELETE FROM transactions WHERE parent_transaction_id = ? OR id = ?', [parentId, parentId]);
            }
            // Se for uma transação principal parcelada (não parcelada), deletar todas as parcelas vinculadas
                ...
                await dbMethods.run('DELETE FROM transactions WHERE parent_transaction_id = ?', [id]);
            }
            // Se for uma parcela, deletar apenas ela
                ...
                const remainingParcels = await dbMethods.get(
                    'SELECT COUNT(*) as count FROM transactions WHERE parent_transaction_id = ? AND id != ?',
                    [transaction.parent_transaction_id, id]
                );

                // Se for a última parcela, deletar também a transação principal
                if (remainingParcels.count === 0) {
                    await dbMethods.run('DELETE FROM transactions WHERE id = ?', [transaction.parent_transaction_id]);
                }
            }
        }
